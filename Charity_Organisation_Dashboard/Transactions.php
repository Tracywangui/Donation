...
// Ensure charity_id is set correctly
if (!isset($_SESSION['charity_id'])) {
    echo "Charity ID is not set in the session.";
    exit;
} else {
    $charity_id = $_SESSION['charity_id'];
    echo "Charity ID from session: " . htmlspecialchars($charity_id); // Debugging line
}
...
// Bind the charity_id
$stmt->bind_param("i", $charity_id); // Ensure this matches the number of placeholders
...
if ($result->num_rows > 0) {
    echo "Transactions found: " . $result->num_rows; // Debugging line
} else {
    echo "No transactions found for charity ID: " . htmlspecialchars($charity_id);
}
?>
...
<head>
    ...
    <title>CharityOrganization - Transactions</title>
                    ...
                    <tbody>
                        <?php while ($row = $result->fetch_assoc()) { ?>
                            <tr>
                                ...
                                <td class="donor-cell"><?php echo htmlspecialchars($row['firstname'] . ' ' . $row['lastname']); ?></td>
            ...
            .then(data => {
                ...
                detailsDiv.innerHTML = `
                    ...
                    <div class="invoice-details">
                        ...
                        <div class="detail-row">
                            <p><strong>Charity:</strong> ${DonorName}</p>
                        </div>
